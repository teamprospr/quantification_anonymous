...
if [[ -d $HOME/bachelor_scriptie/jobs/${job}/results/ ]]; then
    cp -r "${HOME}/bachelor_scriptie/jobs/${job}/results" "${TMPDIR}/${job}/"
fi

# Export scratch path for python script.
export LISA_DIR="$TMPDIR"

timeout ${MAX_TIME} python3 "${HOME}/msc_thesis/run_program.py" \
    dfs_bnb_parallel -j ${job} -l -le 25 -ds vanEck1000

# Copy results from scratch to home.
cp -r "${TMPDIR}/${job}/results" "${HOME}/msc_thesis/jobs/${job}/"
